...
std::shared_ptr<Stmt> Parser::statement()
{
    ...
    if(this->match(RETURN)) return return_statement();
    if(this->match(LEFT_BRACE)) return ANG<BlockStmt>(block());
...
std::shared_ptr<Stmt> Parser::for_statement()
{
    ...
    if(condition == nullptr) condition = ANG<Literal>(true);
...
std::shared_ptr<Expression> Parser::call()
{
    ...
    while(true)
    {
        if(match(LEFT_PAREN)) expr = finish_call(expr);
...
{
    if(match(FALSE)) return ANG<Literal>(false);
    if(match(TRUE)) return ANG<Literal>(true);
    if(match(NULL)) return ANG<Literal>(nullptr);

    if(match(NUMBER, STRING)) return ANG<Literal>(previous().literal);
    if(match(IDENTIFIER)) return ANG<Variable>(previous());
    ...
    if(match(TokenType::THIS)) return ANG<This>(previous());
    ...
    if(match(LEFT_BRACE))
    {
        ...
        if(!check(RIGHT_BRACE))
        {
            do
            {
                ...
                if(match(STRING)) key = ANG<Literal>(previous().literal);
                else if(match(IDENTIFIER)) key = ANG<Literal>(previous().lexeme);
                else throw error(peek(), "Expected string or identifier as dictionary key");
